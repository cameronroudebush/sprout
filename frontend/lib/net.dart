...
class _NetWorthWidgetState extends State<NetWorthWidget> {
  ...
  void initState() {
    ...
    Provider.of<AccountAPI>(
      context,
      listen: false,
    ).accountsUpdated.on().listen((_) => _setNetWorth());
  }
  ...
  Future<void> _setNetWorth() async {
    ...
    try {
      final transactionAPI = Provider.of<TransactionAPI>(
        context,
        listen: false,
      );
        ...
        _netWorthOT?.historicalData.entries
            .where((entry) {
              final cutoffDate = DateTime.now().subtract(
                _getDurationForRange(_selectedChartRange),
              );
              return entry.key.isAfter(cutoffDate) ||
    ...
    return Center(
      child: ToggleButtons(
        isSelected: ChartRange.values
            .map((range) => range == _selectedChartRange)
            .toList(),
        onPressed: (int index) =>
            setState(() => _selectedChartRange = ChartRange.values[index]),
