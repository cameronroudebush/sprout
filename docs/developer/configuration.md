...
| `server.publicUrl`          | `sprout_server_publicUrl`          |                  | The public URL where Sprout is hosted (e.g., https://sprout.my-domain.com). Highly recommended in production to protect against Host Header Injection attacks. |
...
| **SnapTrade**                     |                                          |                                |                                                                                                                                               |
| `providers.snapTrade.enabled`     | `sprout_providers_snapTrade_enabled`     | `true`                         | If this provider is enabled to execute syncs in the background or add accounts.                                                               |
| `providers.snapTrade.clientId`    | `sprout_providers_snapTrade_clientId`    |                                | The Client ID provided by SnapTrade for API access.                                                                                           |
| `providers.snapTrade.consumerKey` | `sprout_providers_snapTrade_consumerKey` |                                | The Consumer Key provided by SnapTrade used for HMAC-SHA256 request signing.                                                                  |
| `providers.snapTrade.rateLimit`   | `sprout_providers_snapTrade_rateLimit`   | `24`                           | How many API calls we allow per day, per user, for this provider.                                                                             |
