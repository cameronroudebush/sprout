...
export class Category extends DatabaseBase {
  ...
  static async getStats(user: User, days: number) {
    const results = await Transaction.getRepository()
      ...
      // Second filter: Must be within the date range
      .andWhere("t.posted >= :startDate", { startDate: subDays(new Date(), days) })
    ...
    const categoryCount: { [name: string]: number } = Object.fromEntries(results.map((x) => [x.category_name, x.total]));
