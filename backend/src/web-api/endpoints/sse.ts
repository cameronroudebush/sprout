...
export class SSEAPI {
  ...
  static sendToSSEUser = new Subject<{ payload: Base; user: User }>();
  ...
  setupSSEListener(req: Request, res: Response, user: User) {
    ...
    SSEAPI.sendToSSEUser.pipe(filter((z) => z.user.id === user.id)).subscribe((x) => {
      const message = RestBody.fromPlain({ payload: x.payload });
