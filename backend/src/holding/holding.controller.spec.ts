...
describe("HoldingController", () => {
  ...
  describe("getSpecificHoldingHistory", () => {
    ...
    it("should throw NotFoundException if history is null in getSpecificHoldingHistory", async () => {
      const holding = TestEntities.holding;
      vi.spyOn(Holding, "findOne").mockResolvedValue(holding);
      netWorthService.getHistoryForHolding.mockResolvedValue(null as any);
